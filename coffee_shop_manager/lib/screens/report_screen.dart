...
class _ReportScreenState extends State<ReportScreen> {
  ...
  List<Widget> _buildUnpaidOrders(List<Order> filteredOrders) {
    final unpaidList = filteredOrders.where((element) => element.status == 'Chưa thanh toán').toList();
    if (unpaidList.isEmpty) {
      return [
        const SizedBox(
          height: 200,
          child: Center(
            child: Text('Không có đơn hàng nào ghi nợ!', style: TextStyle(color: Colors.grey)),
          ),
        )
      ];
    }
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    'Tiền: ${order.total.toString().replaceAllMapped(RegExp(r'(\d{3})(?=\d)'), (Match m) => '${m[1]}.')} đ',
                    style: const TextStyle(color: Color(0xFF5D4037), fontWeight: FontWeight.w600, fontSize: 13),
                  ),
                ],
              ),
              ElevatedButton(
                ...
                child: const Text('Thu nợ', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
        ),
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        final filteredOrders = orders.where((order) {
          if (_timeFilter == 'Hôm nay') {
            final now = DateTime.now();
            return order.date.year == now.year && order.date.month == now.month && order.date.day == now.day;
          } else if (_timeFilter == 'Tuần này') {
            final now = DateTime.now();
            final startOfWeek = now.subtract(Duration(days: now.weekday - 1));
            final endOfWeek = startOfWeek.add(const Duration(days: 7));
            return order.date.isAfter(startOfWeek.subtract(const Duration(seconds: 1))) &&
                order.date.isBefore(endOfWeek);
          }
          return true; // All
        }).toList();
        ...
        for (var o in filteredOrders) {
          for (var item in o.items) {
            drinkSalesCounts[item.drink.name] = (drinkSalesCounts[item.drink.name] ?? 0) + item.quantity;
          }
        }
                  ...
                  children: [
                    ...
                    DropdownButton<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(value: 'Tất cả', child: Text('Tất cả thời gian')),
                      ],
                      onChanged: (val) {
                        if (val != null) {
                          setState(() {
                              ...
                              children: [
                                ...
                                if (top5BestSellers.isEmpty)
                                  const SizedBox(
                                    height: 200,
                                    child: Center(
                                      child: Text('Chưa có dữ liệu bán hàng trong kỳ này', style: TextStyle(color: Colors.grey)),
                                    ),
                                  )
                                else
                                  ListView.builder(
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            Container(
                                              ...
                                              height: 28,
                                              decoration: BoxDecoration(
                                                color: idx == 0 ? const Color(0xFFD2B48C) : const Color(0xFFFAF5EF),
                                                shape: BoxShape.circle,
                                              ),
                                              child: Center(
                                                child: Text(
                                                  ...
                                                  style: TextStyle(
                                                    fontWeight: FontWeight.bold,
                                                    color: idx == 0 ? Colors.white : const Color(0xFF5D4037),
                                                  ),
                                                ),
                                              ),
                                            ),
